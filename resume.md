...
Developed two Angular UIs associated with the optimizer, including a parameter UI that enabled business users to edit optimizer settings such as maximum number of bonds included in the universe. Implemented a permission system that restricted certain settings to manager-level users only. The implementation included backend authentication and authorization detection, and frontend components that disabled editing of restricted settings.

#### Securitized Assets Data Pipeline & Reporting System

Built a C# API that consumed data from Kanerai's Securitized Asset API and aggregated asset, deal, tranche, and manager-level aggregate data. The API queried historical data up to 3 years, backfilled the data universe, saved it to databases, and computed custom business metrics including 4-month tail percentages and historical asset weighted average prices.
...
Maintained and refactored an application that processed raw text files containing market Rate Lock data from multiple vendors including Morgan Stanley, Bank of America, and Citi. The legacy code used manual parsing for each vendor according to a fixed format. Vendor file formats were inconsistent and had evolved over time, making rule-based parsing unreliable.
...
**Work Project | November 2023 – November 2025**
...
**Mod | September 2025 | [GitHub Repository](https://github.com/WilliamOutlawMiller/WarhammerOverhaul)**
...
**Personal Project | November 2025**

Built a responsive portfolio website using ASP.NET Core with Razor Pages for server-side rendering. Implemented dynamic resume parsing from Markdown files with custom parsing logic for narrative content.

Created interactive UI components with CSS animations, scroll-triggered effects, and particle backgrounds. Designed timeline visualization for work experience with animated markers and hover effects.

Developed custom routing for clean URLs and implemented file download functionality for resume export. Utilized modern CSS features including gradients, transforms, and Intersection Observer API for performance.
